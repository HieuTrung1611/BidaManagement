...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public DashboardOverviewDTO getDashboardOverview(Long branchId) {
        ...
        // Doanh thu hôm nay
        List<Object[]> todaySummaryList = invoiceRepository.getSummaryByDateRange(branchId, startOfToday, endOfToday);
        ...
        double revenueToday = todaySummary != null && todaySummary[0] != null
                ? ((Number) todaySummary[0]).doubleValue() : 0.0;

        // Doanh thu tháng này
        LocalDateTime startOfMonth = LocalDate.now().withDayOfMonth(1).atStartOfDay();
        List<Object[]> monthSummaryList = invoiceRepository.getSummaryByDateRange(branchId, startOfMonth, endOfToday);
        ...
        double revenueThisMonth = monthSummary != null && monthSummary[0] != null
                ? ((Number) monthSummary[0]).doubleValue() : 0.0;
    ...
    public RevenueStatisticsDTO getMonthlyRevenue(Long branchId, int year, int month) {
        ...
        List<ChartDataPointDTO> chartData = dailyData.stream()
                .map(row -> ChartDataPointDTO.builder()
                        .label(row[0].toString().substring(8, 10) + "/" + row[0].toString().substring(5, 7))
    ...
    public RevenueStatisticsDTO getWeeklyRevenue(Long branchId, int year) {
        List<Object[]> weeklyData = invoiceRepository.getWeeklyRevenue(branchId, year);
