...
const notificationConfig = {
    ...
    loading: {
        ...
        gradient:
            "from-neutral-100/60 to-transparent dark:from-neutral-900/20 dark:to-transparent",
    },
};
...
const Notification: React.FC<NotificationProps> = ({
    ...
    return (
        ...
        <motion.div
            ...
            className={`relative w-full max-w-sm rounded-xl p-4 backdrop-blur-xl bg-white/15 dark:bg-black/15 border border-neutral-300/60 dark:border-neutral-700/60 overflow-hidden ring-1 ring-neutral-200/40 dark:ring-neutral-700/40 drop-shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105`}>
            <div
                className={`absolute top-0 left-0 h-full w-full bg-linear-to-br ${config.gradient} opacity-50`}></div>
            <div className="relative z-9999 flex items-center space-x-4">
                ...
                <div className="flex-1">
                    <p className="font-normal text-neutral-900 dark:text-neutral-100 text-lg">
                        {title}
                    </p>
                    {message && (
                ...
                <button
                    onClick={onClose}
                    className="shrink-0 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-100 transition-colors p-1.5 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800">
                    <CloseIcon className="h-5 w-5" />
                </button>
            </div>
            {duration && ( // Render progress bar only if duration is provided
                <div className="absolute bottom-0 left-0 h-1 w-full bg-neutral-300/50 dark:bg-neutral-600/50 rounded-b-xl overflow-hidden">
                    <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: "100%" }}
                        transition={{
                        ...
                        className={`h-full bg-linear-to-r from-green-400 via-blue-400 to-sky-400 dark:from-green-500 dark:via-blue-500 dark:to-sky-500`}></motion.div>
                </div>
            )}
        </motion.div>
    );
